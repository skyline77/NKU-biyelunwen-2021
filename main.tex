\documentclass[zihao = -4, linespread = 1.5]{ctexart} % 12pt
\usepackage{docmute}
\input{preamble}
...
\includepdf[pages=1,pagecommand={},width=\textwidth,trim=28mm 25.4mm 28mm 25.4mm, clip]{cover1.pdf} % 左 下 右 上
\includepdf[pages=1,pagecommand={},width=\textwidth,trim=28mm 25.4mm 28mm 25.4mm, clip]{cover2.pdf}

\pagenumbering{Roman}
\begin{zhAbstract}
此模板根据\href{http://jwc.nankai.edu.cn/bylwwsjw/list.htm}{《南开大学本科毕业论文（设计）指导手册2018 》}（以下简称《指导手册》）的要求制作，这是目前最新的版本。
希望此模板能帮助更多正在写论文的同学。
由于作者专业的原因，本模板花较大篇幅展示了如何排版数学公式、定理和证明。如需要对别的内容进行排版（如代码、交换图、化学方程式等），也欢迎联系本人以对此模板进行扩充。
最后，由于本人水平有限，有些地方仍有不足，欢迎指出，本人将会\sout{量力而为}尽力完善。
联系方式：
\begin{description}
...
\begin{itemize}
  \item 下载方式：
    \begin{enumerate*}[label=(\arabic*)]
      ...
      \item Github。
    \end{enumerate*}
    \item 给新手的使用教程：Bilibili。
  \item 请使用XeLaTeX进行编译。
  \item 若不希望将超链接上色，将\texttt{preamble.tex}中 
            \texttt{\textbackslash usepackage\{hyperref\}}的\texttt{colorlinks}选项去掉（位于文件约190行）。
  \item 需要自行在\texttt{cover1.docx}中制作封面并导出为PDF文件，替换 \href{./cover1.pdf}{\texttt{cover1.pdf}}。（不要使用作者的封面\blacksmiley{}）
\end{itemize}

最后，由于作者水平有限，本模板不能完全符合要求，但作者擅长推卸责任且不打算对此负责\smiley{}。
...
使用 \texttt{\textbackslash footnote}输入脚注\footnote{这是脚注。 }。使用 \texttt{\textbackslash cite}引用文献\cite{PangQingShan}（引用之前需要在\nameref{sec:bib}中定义）。
...
另外，你也可以使用 \texttt{cleveref}包的 \texttt{\textbackslash cref}功能，本模板对该命令进行了汉化。
示例：在下文中你可以看到\cref{tab:notation1}、\cref{fig:scatter}、
\cref{eq:1}、\cref{def:supremum}、\cref{thm:monotone}、\cref{algo:1}、\cref{code:fibonacci}。使用传统的 \texttt{\textbackslash ref}还是
较智能的 \texttt{\textbackslash cref}，留给使用者选择。



\subsection{图片}
\cref{fig:scatter} 插入了一个矢量图，经过放大也不会失真。个人的经验是，将图形输出为PDF格式，能够较好的支持中文。将图片输出为其它矢量格式时，可能会导致中文乱码。
\begin{figure}[htpb]
  \centering
...
\cref{fig:left} 和\cref{fig:right} 是两张并排的图片。\cref{fig:WuHeQiLin} 含有4个子图。
\begin{figure}[htbp]
\centering
\begin{minipage}[t]{0.48\textwidth}
\centering
  \includegraphics[width=0.95\textwidth]{picture/left.pdf}
  \caption{公司最优生产速率}
...
\begin{minipage}[t]{0.48\textwidth}
\centering
  \includegraphics[width=0.95\textwidth]{picture/right.pdf}
  \caption{公司的最优交易速率}
  \label{fig:right}
\end{minipage}
\end{figure}
...
\clearpage
\subsection{表格}
\cref{tab:notation1} 用三线表的形式进行符号说明。
\begin{table}[htbp]
\centering
\caption{符号表示1}
\label{tab:notation1}
...
\cref{tab:ZhangSan1} 也是三线表，但与前者不同，它是二维的，“姓名”和“学科”唯一决定了成绩。将其用\cref{tab:ZhangSan2} 展示可以更清晰的看到这一点。
...
使用\texttt{equation}环境得到带编号公式，如\cref{eq:1} 所示。
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
注意到虽然中间有一行字，但\cref{eq:textUp} 和\cref{eq:textLow} 仍在同一个数学公式中。
...
\begin{equation}\label{eq:aligned}
  ...
  \end{aligned}
\end{equation}
注意到它只会对整个公式居中编号，而不像\cref{eq:align} 对每行都进行编号。
...
下面这种if-else情况可以用\texttt{case}环境生成：
\begin{equation}
  f(x)
    = \begin{cases}
    -x^{2},            &\text{$x < 0$;}\\
...
从\cref{def:supremum} 容易看到，如果数集$S$有上（下）确界，则它的上（下）确界是唯一的。以上定义中sup是supermum的简写，而inf是infimum的简写。设$\beta = \sup S$，这包含两层意思：\begin{enumerate*}[label=(\roman*)]
...
\section{数字和单位}
\cref{tab:number} 展示了一些数字和单位的写法，以及常见的错误写法。
\begin{table}[htbp]
\centering
...
\cref{code:fibonacci} 用Python实现了通过递推的方式计算斐波那契数列，以防止栈溢出。
...
我们可以使用 \href{https://ctan.org/pkg/algorithms}{\texttt{algorithms}} 包排版伪代码。\cref{algo:1} 将指数计算的复杂度由$O(n)$降到了$O(\log n)$。
\begin{algorithm}
  \caption{加速指数计算}
  \label{algo:1}
  \begin{algorithmic}
...
% 由于NK应该是只需要一个目录，因此采用了下面的方法，而不是真正的 appendix

\setcounter{secnumdepth}{0} % 抑制section的编号
\section{\texorpdfstring{附\quad 录}{附录}}
%\section{附\quad 录} % 正常情况下应该这么写，但为了兼容PDF书签功能，使用了上面的写法
...
\bibitem{url}SREC Trade, New jersey srec market, 2019, \url{https://www.srectrade.com/srec markets/new jersey} (accessed 2019-02-13).
\end{thebibliography}
...
%\section{致\quad 谢} % 正常情况下应该这么写，但为了兼容PDF书签功能，使用了上面的写法
感谢使用本模板。
